...
class ArtesaPipeline:

    # Database setup
    conn = sqlite3.connect('artesa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `artesa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM artesa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO artesa (date, title, link, content)"
